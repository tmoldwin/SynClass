...
def preprocess_synapse_2d(data_3d, pre_mask_3d, post_mask_3d, input_size=224, augment=False, seed=None):
    """Convert 3D synapse volume + masks to 2D multi-channel tensor.
    
    Generic preprocessing: middle slice, optional augmentation, resize, normalize.
    Output: (3, H, W) tensor with channels [EM_image, pre_mask, post_mask].
    EM channel uses ImageNet normalization; masks are [0, 1].
    
    Args:
        data_3d: (H, W, Z) raw EM volume
        pre_mask_3d: (H, W, Z) pre-synaptic mask
        post_mask_3d: (H, W, Z) post-synaptic mask
        input_size: Target spatial size (H, W)
        augment: Whether to apply augmentation
        seed: Random seed for augmentation (for reproducibility)
        
    Returns:
        torch.Tensor of shape (3, input_size, input_size)
    """
    import cv2
    ...
    return torch.cat([data_tensor, pre_tensor, post_tensor], dim=0)
